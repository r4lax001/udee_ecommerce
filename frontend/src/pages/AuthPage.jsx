...
const getStrengthText = (score) => {
  ...
  if (score === 3)
    return { text: "รหัสผ่านปลอดภัยปานกลาง", color: "text-[#7F5530]" };
...
const AuthPage = ({ authData = authPageData }) => {
  ...
  const handleLoginSubmit = () => {
    ...
    if (!savedUser) {
      setErrorMessage("ยังไม่มีบัญชีผู้ใช้นี้ กรุณาสมัครสมาชิกก่อน");
      return;
    }

    if (
      savedUser.email !== loginEmail.trim() ||
  ...
  const handleRegisterSubmit = () => {
    ...
    localStorage.setItem("udee_user", JSON.stringify(newUser));
